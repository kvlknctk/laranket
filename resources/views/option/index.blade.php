    ...
    <div class="row">
        @forelse($polls as $item)
            ...
            <div class="col-md-3" style="border: 1px solid #c6c6c6; margin: 5px; padding: 5px;">
                ...
                <form action="{{route('secenek.store')}}" method="POST" enctype="multipart/form-data" style="padding: 10px;zoom: 0.7; background-color: #e5ffbe; border: 1px solid #000;"  >
                    @csrf
                    <input type="hidden" name="poll_id" value="{{$item->id}}">
                    <input type="file" name="image" placeholder="resim seç" class="form-control">
                    <input type="text" name="color" placeholder="Örnek : #FF00FF" class="form-control" autocomplete="off">
                    <input type="text" name="title" placeholder="Seçenek ismi" class="form-control" autocomplete="off">
